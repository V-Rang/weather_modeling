...
def train_one_epoch():
    ...
    for i, data in enumerate(tqdm(train_loader)):
        ...
        inputs = inputs.reshape(batch_size, block_size, -1)
        true_outputs = true_outputs.reshape(batch_size, block_size, -1)
        optimizer.zero_grad()
...
for epoch in range(EPOCHS):
    ...
    with torch.no_grad():
        for i, vdata in enumerate(val_loader):
            ...
            vinputs = vinputs.reshape(batch_size, block_size, -1)
            vlabels = vlabels.reshape(batch_size, block_size, -1)
    ...
    if avg_vloss < best_vloss:
        ...
        model_name = 'models/model_{}_{}'.format(timestamp, epoch_number)
        torch.save(model.state_dict(), model_name)

    epoch_number += 1
    
#plot of training and validation losses:
figures_path = f"Figures/{model_name[7:]}/" 
